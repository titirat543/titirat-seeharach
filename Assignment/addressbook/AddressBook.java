 ...
 *
 * THIS STARTER CODE WORKS PARTIALLY BUT SOME METHODS REQUIRE COMPLETING 
 * - SEE FURTHER DOWN IN THIS COMMENT
 *
...
public class AddressBook extends JFrame implements ActionListener
{
    ...
    public static void main(String[] args)
    {
        ...
        contacts.setTitle("My address book: 2636568");
    ...
    private void deleteContact(int index)
    {
        ...
        for(int i = 0;i< currentSize ;i++)
        {
            if(index == currentContact){
                name[currentContact] = " ";    //set name which we just choose to delete
                address[currentContact] = " "; //set address which we just choose to delete
                mobile[currentContact] = " "; //set mobile which we choose to delete
                email[currentContact] = " "; //set email which we choose to delete
            }
        }
    ...
    private void clearContacts()
    {
        ...
        for(int i=0;i<currentSize;i++)
        {   
            name[i]=null;   //set name data to emtry
            address[i]=null;  //set address data  to emtry
            mobile[i]=null; //set mobile data to emtry
            email[i]=null;  //set email data to emtry
    ...
    {
        // TO BE DONE: Implement this method body - see comments above
        for(int i = 0;i< currentSize ;i++) // Transver 
        {
            if(name[i].equals(searchName)){ // when we found the name which we want
                return i;
            }
        }
    ...
    private int findPartial(String searchText)
    {
        ...
        for(int i = 0;i< currentSize ;i++) //Transver
        {
            if(name[i].contains(searchText)){ // when we input one text and we have in the addressbook
                return i;
            }
        }
        return -1;                          // Return where found or -1
     ...
     */
    private static void swapAlphabet(String[]name,int i,int j) 
     {
            String temp = name[i];
            name[i]= name[j];
            name[j]= temp;
    ...
    private void sortAtoZ()
    {
        // TO BE DONE: Implement this method body - see comments above
            ...
            for(int j =i+1 ;j<currentSize;j++){
                ...
                {   
                    swapAlphabet(name,i,j);
                    swapAlphabet(address,i,j);
                    swapAlphabet(mobile,i,j);
                    swapAlphabet(email,i,j);
                }
            }
        }
        }
